...
# Add local 'lib' folder to sys.path to find BAT
addon_dir = Path(__file__).parent
...
class SevenZipManager:
    ...
    def _locate_binary(self):
        """Locate appropriate 7-Zip binary based on platform/arch"""
        ...
        if not binaries_dir.exists():
            print("[BlendPack] 7z_binaries folder not found. Falling back to zipfile.")
            return
        ...
        if system == "windows":
            ...
            if machine in ["arm64", "aarch64"]:
                binary_path = binaries_dir / "windows" / "arm64" / exe_name
            else:
                # Default to x64 for amd64 or x86
                binary_path = binaries_dir / "windows" / "x64" / exe_name
                
        elif system == "linux":
            exe_name = "7zz"
            if machine in ["arm64", "aarch64"]:
                binary_path = binaries_dir / "linux" / "arm64" / exe_name
            else:
                binary_path = binaries_dir / "linux" / "x64" / exe_name
                
        elif system == "darwin": # macOS
            exe_name = "7zz"
            # macOS binary in your tree seems universal or single folder
            binary_path = binaries_dir / "darwin" / exe_name

        # 2. Verify and Permission
        if binary_path and binary_path.exists():
            ...
            if system in ["linux", "darwin"]:
                try:
                    ...
                    print(f"[BlendPack] Failed to chmod 7zip: {e}")
            
            print(f"[BlendPack] Using 7-Zip: {binary_path}")
        else:
            print(f"[BlendPack] 7-Zip binary not found for {system}/{machine} at {binary_path}")

    def compress(self, source_dir, output_zip, progress_callback=None):
        """
        Compress using 7-Zip subprocess with progress parsing.
        Returns True if successful.
        """
        if not self.use_7zip:
            return False

        try:
            # Command: 7za a -tzip -mx5 -mmt -bsp1 output.zip source_dir/*
            cmd = [
                str(self.binary_path),
                "a",                    # Add
                "-tzip",                # Format: Zip
                ...
                "-y",                   # Yes to all
                str(output_zip),
                str(source_dir / "*")   # Wildcard to zip contents, not folder
            ]

            # subprocess.STARTUPINFO to hide window on Windows
            startupinfo = None
            ...
            if return_code != 0:
                ...
                print(f"[BlendPack] 7-Zip Error: {err}")
                return False
            
            return True

        except Exception as e:
            print(f"[BlendPack] 7-Zip Exception: {e}")
            return False
...
class BlendpackProperties(PropertyGroup):
    output_path: StringProperty(
        name="Output Path",
        description="Folder where the packed zip will be created",
        default="",
        subtype='DIR_PATH'
    )
    progress: FloatProperty(
...
class PackingTask:
    """
    Handles the logic in a separate thread.
    1. BAT (Analysis/Copy)
    2. 7-Zip (Compression)
    """
    
    def __init__(self, blend_path_str, output_path_str):
        self.blend_path = Path(blend_path_str)
    ...
    def run(self):
        ...
        try:
            if not BAT_AVAILABLE:
                raise ImportError(f"BAT Library not loaded. {BAT_ERROR}")

            self._do_packing()
            
        except Exception as e:
            self.error = str(e)
            print("BlendPack Error Traceback:")
            traceback.print_exc()
        finally:
            self.cleanup()
            self.finished = True

    def _do_packing(self):
        ...
        # 2. Setup BAT Packer
        self.update_status(10.0, "BAT: Analyzing dependencies...")
        
        packer = bat_pack.Packer(
            bfile=self.blend_path,
            project=self.blend_path.parent,
            target=temp_pack_target
        )
        
        # 3. Strategise (Trace)
        packer.strategise()
        
        # 4. Execute (Copy/Relink)
        self.update_status(40.0, "BAT: Copying assets & Relinking...")
        packer.execute()
        
        # Check for missing files
        if hasattr(packer, 'missing_files') and packer.missing_files:
            print(f"[BlendPack] BAT reported {len(packer.missing_files)} missing files.")

        # 5. Zip the result
        self.update_status(70.0, "Compressing archive...")
        
        zip_filename = f"{self.blend_path.stem}_packed.zip"
        final_zip_path = self.output_path / zip_filename
        
        # Try 7-Zip first, fallback to Python Zip
        success = False
        if self.seven_zip.use_7zip:
            success = self.seven_zip.compress(
                temp_pack_target, 
                final_zip_path, 
                progress_callback=self._seven_zip_progress
            )
        
        if not success:
            print("[BlendPack] 7-Zip failed or unavailable. Using Python Zipfile.")
            self._create_zip_fallback(temp_pack_target, final_zip_path)
        
        self.zip_created_path = final_zip_path
        self.update_status(100.0, "Packing completed")

    def _seven_zip_progress(self, percent):
        # Map 7-Zip's 0-100% to our global 70-100%
        # 70 + (percent * 0.3)
        global_prog = 70.0 + (percent * 0.3)
        self.update_status(global_prog, f"Archiving... {int(percent)}%")

    def _create_zip_fallback(self, source_dir, output_zip_path):
        """Python fallback for zipping"""
    ...
    def update_status(self, progress, message):
        ...
        self.status = message
        # print(f"[BlendPack] {progress:.1f}%: {message}")
    ...
    def cleanup(self):
        ...
        try:
            ...
            print(f"[BlendPack] Cleanup warning: {e}")
...
class BLENDPACK_OT_select_folder(Operator):
    bl_idname = "blendpack.select_folder"
    bl_label = "Select Folder"
    bl_description = "Select where to save the packed zip file"
    bl_options = {'INTERNAL'}
...
class BLENDPACK_OT_start_packing(Operator):
    bl_idname = "blendpack.start_packing"
    bl_label = "Start Packing"
    bl_description = "Analyze and pack project using BAT"
    
    _timer = None
    _thread = None
    _task = None
    _finish_time = None  # NEW: Track when task finished
    _reset_delay = 1.0   # NEW: Delay in seconds before hiding progress
        ...
        if event.type == 'TIMER':
            ...
            if self._task:
                ...
                if self._task.finished:
                    # NEW: Handle delayed reset
                    if self._finish_time is None:
                        # Just finished - record the time
                        import time
                        ...
                        if self._task.error:
                            props.error_message = self._task.error
                            props.status_message = "Failed"
                            self.report({'ERROR'}, f"Packing Failed: {self._task.error}")
                        else:
                            # FORCE PROGRESS TO 100 ON COMPLETION
                            props.progress = 100.0
                            props.error_message = ""
                            props.status_message = "Packing completed"
                            
                            zip_name = self._task.zip_created_path.name if self._task.zip_created_path else "file"
                            self.report({'INFO'}, f"Successfully packed: {zip_name}")
                    
                    else:
                        # NEW: Check if delay has elapsed
                        import time
                        elapsed = time.time() - self._finish_time
    ...
    def execute(self, context):
        ...
        # Validation
        if not BAT_AVAILABLE:
            self.report({'ERROR'}, "BAT Library is missing. Cannot pack.")
            return {'CANCELLED'}
        ...
        # NEW: Reset finish time tracker
        self._finish_time = None
        
        # Init Task
        self._task = PackingTask(bpy.data.filepath, props.output_path)
        
        # Start Thread
        self._thread = threading.Thread(target=self._task.run, daemon=True)
        self._thread.start()
...
class BLENDPACK_PT_main_panel(Panel):
    ...
    def draw(self, context):
        ...
        if not BAT_AVAILABLE:
            box = layout.box()
            box.alert = True
            box.label(text="Library Missing!", icon='ERROR')
            box.label(text="Ensure 'lib' folder exists")
            box.label(text=f"Err: {BAT_ERROR[:30]}...")
            return
        ...
        if props.output_path:
            row.label(text=props.output_path, icon='FOLDER_REDIRECT')
        else:
            row.label(text="Select Output Folder...", icon='FOLDER_REDIRECT')
            
        col.operator("blendpack.select_folder", text="Browse", icon='FILEBROWSER')
        ...
        if props.is_processing:
            row.operator("blendpack.start_packing", text="Packing...", icon='TIME')
        else:
            row.operator("blendpack.start_packing", text="Start Packing", icon='EXPORT')
        ...
        if props.show_progress:
            ...
            col = box.column(align=True)
            col.prop(props, "progress", text="Progress", slider=True)
            col.label(text=props.status_message)
        ...
        if props.error_message:
            ...
            box.alert = True
            col = box.column()
            col.label(text="Error Occurred:", icon='ERROR')
            
            # Basic text wrapping
            err_text = props.error_message
            while len(err_text) > 0:
                col.label(text=err_text[:40])
                err_text = err_text[40:]
...
def register():
    for cls in classes:
        bpy.utils.register_class(cls)
    bpy.types.Scene.blendpack_props = PointerProperty(type=BlendpackProperties)
    if load_handler not in bpy.app.handlers.load_post:
        bpy.app.handlers.load_post.append(load_handler)
    print("BlendPack v1.1 Registered")
...
def unregister():
    ...
    for cls in reversed(classes):
        bpy.utils.unregister_class(cls)
    print("BlendPack v1.1 Unregistered")
